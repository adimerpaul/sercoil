...
<div class="box">
	...
	<div class="box-body">
						...
						<th>
							<form action="{{route('venta.eliminarproducto')}}" method="POST">
								...
								<div class="col-md-6">
									...
									<button type='submit' data-row='row{{$numero+1}}' class='btn btn-danger btn-xs remove-add-nuevogrupocarrera_1'><span class='fa fa-times'></span>
								</div>
							</form>
			...
			<form method="POST" action="{{route('venta.empleado')}}" parsley-validate novalidate>
				...
				<table class="table table-bordered">
					<tr class="info">
						<td>
							<div class="form-group">
								<select class="form-control" name="id_empl" required>
									<option selected="" disabled="">-ASIGNAR EMPLEADO-</option>
						...
						<td>
							<button type="submit" class="btn btn-info btn-block"><i class="fa fa-user"></i> ASIGNAR EMPLEADO</button>
						</td>
					</tr>
				</table>
			</form>
		...
		<form action="{{route('venta.finaliza')}}" method="POST">
			{{csrf_field()}}
			<input type="hidden" name="id_ven" value="{{$venta->ID_VEN}}">
			<div class="col-md-6"><button type="submit" class="btn btn-danger btn-block"><i class="fa fa-check"></i> FINALIZAR VENTA</button></div>
		</form>
		@endif
	</div>
</div>
...
<script type="text/javascript">
	function cantidad(input){
		if (input.value>parseInt($('#uni_dis').val())) {input.value=$('#uni_dis').val()}
			var cantidad=parseInt($('#cant_pro').val());	
