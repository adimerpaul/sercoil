...
@section('li_personalizada','active')
@section('title','COTIZACIÓN PERSONALIZADA')
@section('content')


<form method="POST" action="{{route('cotizacion_personal.guarda')}}" id="form_cotizacion" parsley-validate novalidate>
	{{csrf_field()}}	
	<div class="col-md-12">
		<!-- Custom Tabs -->
	...
	<script type="text/javascript">
		function calcular(){
			...
			var route= "{{route('cotizacion_personal.calcula')}}";
			$.ajax({
				url: route,           
	...
	<script type="text/javascript">
		function guardar(){
			var base=$('#base').val();
			var altura=$('#altura').val();
			var route= "{{route('cotizacion_personal.guarda')}}";
			$.ajax({
				url: route,           
				headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
				type: 'POST',     
	...
	<script type="text/javascript">
		function guardado(){
			var base=$('#base').val();
			var altura=$('#altura').val();
			var route= "{{url('/guardado_personal')}}";
			$.ajax({
				url: route,           
				headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
				type: 'GET',     
					...
					$.ajax({
						...
						success: function(data){
							...
							if (data!='FALSE') {
								...
								$('#div_botonera').html('<a href="cotizacion_personal-pdf/'+data.ID_COT+'" target="_blank" class="btn btn-danger"><i class="fa fa-print"></i> IMPRIMIR PROFORMA GUARDADA</a>');
							}else{
								$.notify({icon:'<i class="fa fa-ban"></i> ',title:' <strong>ERROR!</strong></br>',message:"{{'  Ocurrio un problema al guardar, recargue la Pagina'}}"},{z_index: 2000, type:'danger',animate:{enter:'animated fadeInDown',exit:'animated fadeOutUp'}});
							}
						},
